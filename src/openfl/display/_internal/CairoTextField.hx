...
class CairoTextField
{
	public static function render(textField:TextField, renderer:CairoRenderer, transform:Matrix):Void
	{
		...
		if (graphics.__bitmapScale != pixelRatio)
		{
			// the TextField might have rendered in a context that requires a
			// different pixel ratio than normal, such as when drawing to
			// BitmapData. Force a re-render so the bitmap is rasterized at the
			// correct resolution, otherwise the text can appear blurry.
