...
class Stage extends DisplayObjectContainer #if lime implements IModule #end
{
	...
	**/
	...
	// ═══════════════════════════════════════════
	// MohongEngine — Render optimization flags
	// ═══════════════════════════════════════════

	/**
		Whether render optimization features (draw-call batching, culling) are enabled.
		Soft-coded toggle for MohongEngine's RenderOptimizer integration.
	**/
	public var renderOptimizationEnabled:Bool = true;

	/**
		Whether GPU texture pooling is active to reduce VRAM fragmentation.
		Soft-coded toggle for MohongEngine's GPUTextureManager integration.
	**/
	public var texturePoolingEnabled:Bool = true;

	/**
		Whether automatic periodic memory cleanup is enabled.
		When true, the stage will trigger cache cleanup on low-memory conditions.
	**/
	public var autoMemoryCleanupEnabled:Bool = true;
	...
	**/
	...
	// ═══════════════════════════════════════════
	// MohongEngine — Memory management API
	// ═══════════════════════════════════════════

	/**
		Trigger a lightweight memory cleanup pass.
		Flushes cached BitmapData that is no longer referenced by the display list,
		and requests a garbage collection hint from the runtime.
		Safe to call every few seconds; designed for MohongEngine's MemoryMonitor.
	**/
	public function performMemoryCleanup():Void
	{
		if (!autoMemoryCleanupEnabled)
			return;

		#if (cpp && lime)
		// Dispatch to the Lime application's render context for texture cleanup
		if (context3D != null)
		{
			try
			{
				context3D.dispose(); // Forces re-creation on next use, clearing VRAM
			}
			catch (e:Dynamic) {}
		}
		#end

		// Trigger native GC hint via Lime
		#if cpp
		cpp.vm.Gc.run(false); // Minor collection — fast
		#end
	}

	/**
		Perform a major memory cleanup — releases all non-essential GPU resources
		and triggers a full garbage collection. Use sparingly (e.g., on state transitions).
	**/
	public function performMajorCleanup():Void
	{
		#if (cpp && lime)
		if (context3D != null)
		{
		...
		#if cpp
		cpp.vm.Gc.run(true);  // Major collection
		cpp.vm.Gc.compact();  // Compact heap
		#end
	}
