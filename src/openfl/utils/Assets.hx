...
class Assets
{
	...
	**/
	...
	// ═══════════════════════════════════════════
	// MohongEngine — Cache management utilities
	// ═══════════════════════════════════════════

	/**
		Get statistics about the current asset cache state.
		Returns an anonymous object with counts for each cached asset type.

		@return Object with fields: bitmapCount, soundCount, fontCount, totalBytes (estimated).
	**/
	public static function getCacheStats():Dynamic
	{
		var stats:Dynamic = {};
		Reflect.setField(stats, "bitmapCount", 0);
		Reflect.setField(stats, "soundCount", 0);
		Reflect.setField(stats, "fontCount", 0);
		Reflect.setField(stats, "totalBytes", 0);

		if (cache != null)
		{
			try
			{
				// Access the underlying AssetCache implementation
				var bitmapCache:Dynamic = Reflect.field(cache, "__bitmapData");
				var soundCache:Dynamic = Reflect.field(cache, "__sound");
				var fontCache:Dynamic = Reflect.field(cache, "__font");

				if (bitmapCache != null)
				{
					var keys:Array<String> = [for (k in Reflect.fields(bitmapCache)) k];
					Reflect.setField(stats, "bitmapCount", keys.length);
				}
				if (soundCache != null)
				{
					var keys:Array<String> = [for (k in Reflect.fields(soundCache)) k];
					Reflect.setField(stats, "soundCount", keys.length);
				}
				if (fontCache != null)
				{
					var keys:Array<String> = [for (k in Reflect.fields(fontCache)) k];
					Reflect.setField(stats, "fontCount", keys.length);
				}
			}
			catch (e:Dynamic) {}
		}
	...
	/**
		Clear all cached assets that are not currently referenced by active display objects.
		Safe to call during loading screens or state transitions.
		Returns the number of assets cleared.
	**/
	public static function clearUnusedCache():Int
	{
		var cleared:Int = 0;
			...
			try
			{
				...
				if (bitmapCache != null)
				{
					...
					for (key in keys)
					{
						try
						{
							var bmp:BitmapData = Reflect.field(bitmapCache, key);
							// Only clear bitmaps that aren't currently bound to display objects
							if (bmp != null && bmp.__worldTransform == null)
							{
								Reflect.deleteField(bitmapCache, key);
								cleared++;
							}
						}
						catch (e:Dynamic) {}
